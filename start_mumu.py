...
class CookingBot:
                        ...
                        for attempt in range(2):
                            ...
                            if menu_buttons:
                                if self.menu_clicks == 0:
                                    ...
                                    # 后续只要检测到按钮就停止旋转
                                    logger.info(
                                        f"=== 检测到 {len(menu_buttons)} 个菜单按钮，已点击 {self.menu_clicks} 次，停止旋转 ===")
                                    return True
                            time.sleep(0.3)
    ...
    def handle_menu_state(self):
        ...
        try:
            ...
            self.save_debug_images('cook_menu')
            ...
            if len(finish_buttons) > 0:
                ...
                try:
                    if self.click_button(finish_buttons[0], 'finish', wait_time=0.3):
            ...
            logger.error(f"处理菜单检测状态时出错: {e}")
            if not self.handle_timeout():
                raise Exception("菜单检测状态处理超时")
            ...
            try:
                ...
                if not self.start_button_pos:
                    ...
                    if isinstance(start_buttons, list) and len(start_buttons) > 0:
                        ...
                        self.start_button_pos = start_buttons[0].tolist() if isinstance(start_buttons[0],
    ...
    def handle_finish_state(self):
        ...
        try:
            ...
            if self.menu_clicks < 3 and finish_buttons and len(finish_buttons) > 0 and self.start_clicks == 0:
                ...
                try:
                    finish_button = finish_buttons[0].tolist() if isinstance(finish_buttons[0], np.ndarray) else list(
                        finish_buttons[0])
                ...
                if finish_buttons and len(finish_buttons) >= self.start_clicks:  # finish按钮数量需要等于start点击次数
                    ...
                    try:
                        ...
                        for finish_button in finish_buttons:
                            button_data = finish_button.tolist() if isinstance(finish_button, np.ndarray) else list(
                                finish_button)
                    ...
                    logger.info(
                        f"未检测到足够的finish按钮（当前: {len(finish_buttons)}, 需要: {self.start_clicks}），继续寻找")
                    if self.rotate_view():
                        # 旋转找到按钮后，再次验证按钮数量
                        finish_buttons = self.detect_buttons('finish')
                        if len(finish_buttons) < self.start_clicks:
                            logger.info(
                                f"旋转后仍未找到足够的finish按钮（当前: {len(finish_buttons)}，需要: {self.start_clicks}），继续寻找")
                        self.reset_rotation()
                    else:
                ...
                if self.food_clicked:
                    ...
                    if not self.food_button_pos:
                        ...
                        if food_buttons:
                            ...
                            self.food_button_pos = food_buttons[0].tolist() if isinstance(food_buttons[0],
                                                                                        np.ndarray) else list(
                                food_buttons[0])
                            self.menu_clicks += 1
                            logger.info(f"当前菜单点击次数: {self.menu_clicks}")
