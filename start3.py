...
def match_template_scaled(icon, gray_screen, threshold, result_queue):
    ...
    try:
        ...
        if use_cuda:
            ...
            masked_template_gpu.upload(gray_template)
        else:
            gray_screen_gpu = gray_screen
            masked_template_gpu = gray_template

        for scale in np.arange(0.7, 1.1, 0.1):  # 缩放范围从 50% 到 110%
    ...
    try:
        ...
        for icon in icons:
            if icon in results:
                ...
                if "cook.png" in icon:
                    time.sleep(0.5)
                elif "food.png" in icon and can_click_food:
                    time.sleep(1)  # 可以点击 cook_start
                    can_click_food = False  # 防止再点击 food
                elif "cook_start.png" in icon and not can_click_food:
                    time.sleep(1)  # 点击 cook_start 后继续下一次
