...
def preprocess_image_white_only(image_path):
    ...
    try:
        ...
        _, mask = cv2.threshold(gray_template, 240, 255, cv2.THRESH_BINARY)
...
def match_template_scaled(icon, gray_screen, threshold, result_queue,screen):
    """ 为每个缩放比例进行模板匹配 """
    try:

        template, gray_template, mask, masked_template = preprocess_image_white_only(icon)

        if "food.png" in icon or "cook_start.png" in icon:
            masked_template=gray_template
...
def handler(window_title):
    ...
    try:
        ...
        # 为每个图标启动一个线程进行缩放匹配
        for icon in icons:
            thread = threading.Thread(target=safe_run_thread, args=(match_template_scaled, icon, gray_screen, threshold, result_queue,screen))
        ...
        for icon in [f"{script_dir}\\cook_menu.png"]:
            if icon in results:
        ...
        for icon in [ f"{script_dir}\\finish.png"]:
            if icon in results:
                match_position, template_size = results[icon]
                left, top = match_position
                width, height = template_size
        ...
        # 其他按钮的点击逻辑
        for icon in [f"{script_dir}\\food.png",f"{script_dir}\\cook_start.png"]:
            if icon in results:  # 排除cook.png
                match_position, template_size = results[icon]
                left, top = match_position
                width, height = template_size
                threading.Thread(target=draw_rectangle, args=(left, top, width, height, 0.5)).start()
